...
"""
    ...
    if character_config.llm_generated_topics and not video_config.topic:
        # Use CONTENT_CATEGORIES to select a random topic
        if character_config.subcategory:
            topics = CONTENT_CATEGORIES[character_config.category][character_config.subcategory]
        else:
            # If no subcategory is specified, collect all topics under the category
            topics = []
            for subcat in CONTENT_CATEGORIES[character_config.category].values():
                topics.extend(subcat)
        video_config.topic = random.choice(topics)

    # Validate the emotion
    if character_config.selected_emotion not in [emotion for emotions in EMOTIONS.values() for emotion in emotions]:
        # If the selected emotion is not valid, pick a random one from the best emotions for the selected story framework
        ...
        if best_emotions:
            ...
            # Pick a random emotion if no best emotions are specified
            video_config.emotion = random.choice([emotion for emotions in EMOTIONS.values() for emotion in emotions])

    # Prepare the hook instruction
    if hook_example:
        hook_line = f'Similar to: "{hook_example}"'
    else:
        hook_line = ''

    # Prepare the outro style line
    if video_config.outro_template:
        outro_style_line = f'- Outro style: "{video_config.outro_template}"'
    else:
        outro_style_line = ''

    # Build the prompt without an example script
    prompt = (
        f"{base_profile}\n\n"
        ...
        f"Instructions:\n"
        f"- Start with a captivating hook. {hook_line}\n"
