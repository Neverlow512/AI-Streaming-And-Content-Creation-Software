...
class ViralCharacterConfig:
    ...
    use_template_hooks: bool  # New field
...
def generate_viral_prompt(
    ...
    video_structure = VIDEO_STRUCTURES[video_config.video_structure]['structure']
    story_framework = STORY_FRAMEWORKS[video_config.story_framework]['structure']
    
    # Get hook templates for the selected type
    hook_templates = HOOK_TYPES[video_config.hook_type]['templates']
    hook_example = random.choice(hook_templates).format(topic=video_config.topic)
    
    # Get outro template if using templates
    if video_config.use_template_outro and video_config.outro_category:
        outro_templates = OUTROS["Template"][video_config.outro_category]
        outro_text = f"End with this outro style: {random.choice(outro_templates)}"
    else:
    ...
    - Hook Type: {video_config.hook_type}
    ...
    - Main Content: Structure according to {video_config.video_structure} format
    - Outro: 3-5 seconds
